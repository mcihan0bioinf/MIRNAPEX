# MLP training with high/low stratified pairing in both directions.
...
def generate_pairs(sample_list, num_pairs, mirna_name, isomir_df):
    """Median split (high/low) and random pairing in BOTH directions within sample_list."""
...
def compute_pairwise_differences(pairs, apa_df, gene_df, isomir_df):
    """Return APA diff, gene log2FC, and miRNA log2FC dataframes with pair-named columns."""
...
def build_X(apa_block, gene_block, apa_index, gene_index):
    """Align *_apa and *_gene rows to fixed indices; fill missing with 0, return pairs x features."""
...
def make_model(input_dim, learning_rate=1e-3):
    """Same architecture/activations as your original."""
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python train_mlp_mirna.py <batch:int> <top_x_genes:int>")
        sys.exit(1)
    ...
    # Split samples (no leakage)
    train_samples, test_samples = train_test_split(samples, test_size=0.2, random_state=42)

    # --- Small CV grids (training-only knobs; model stays the same) ---
    learning_rates = [1e-3, 3e-4,1e-3]
    ...
    for idx, miRNA in enumerate(mirnas, start=1):
        ...
        # ---- 5-fold CV over (learning_rate, batch_size, patience) ----
        best_params = {"learning_rate": None, "batch_size": None, "patience": None}
            ...
            for bs in batch_sizes:
                for pat in patiences:
                    ...
                    for i in range(5):
                        ...
                        model = make_model(X_tr_s.shape[1], learning_rate=lr)
                        es = keras.callbacks.EarlyStopping(monitor="val_loss", patience=pat, restore_best_weights=True)
                        model.fit(
                            X_tr_s, y_tr,
                            validation_split=0.1,  # from training only (no leakage)
                            epochs=200,
                            batch_size=bs,
                            verbose=0,
                            callbacks=[es],
                        ...
                        y_hat = model.predict(X_val_s, verbose=0).flatten()
                    ...
                    if avg_r2 > best_r2:
                        ...
                        best_params = {"learning_rate": lr, "batch_size": bs, "patience": pat}

        # ---- Train final model on all folds with best params ----
        ...
        final = make_model(X_train_all_s.shape[1], learning_rate=best_params["learning_rate"])
        es_final = keras.callbacks.EarlyStopping(monitor="val_loss", patience=best_params["patience"], restore_best_weights=True)
        final.fit(
            X_train_all_s, y_train_all,
            validation_split=0.1,  # from training only
            epochs=200,
        ...
        # Test metrics
        y_pred = final.predict(X_test_s, verbose=0).flatten()
        all_results.append({
            "miRNA Name": miRNA,
            "Best learning_rate": best_params["learning_rate"],
            "Best batch_size": best_params["batch_size"],
            "Best patience": best_params["patience"],
            "MLP Test R² Score": r2_score(y_test, y_pred),
            "MLP Test MSE": mean_squared_error(y_test, y_pred),
            "MLP Test Mean Absolute Error": mean_absolute_error(y_test, y_pred),
            "MLP Test Median Absolute Error": median_absolute_error(y_test, y_pred),
    ...
    # Save
    out_csv = out_dir / "final" / f"mlp_n_{n}_batch_{batch}_top{top_x_genes}.csv"
