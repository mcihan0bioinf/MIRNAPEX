# Ridge training with high/low stratified pairing in both directions.
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python train_ridge_mirna.py <batch:int> <top_x_genes:int>")
        sys.exit(1)
    ...
    for idx, miRNA in enumerate(mirnas, start=1):
        ...
        final = Ridge(alpha=best_alpha, max_iter=1000)
    ...
    out_csv = out_dir / "final" / f"ridge_n_{n}_batch_{batch}_top{top_x_genes}.csv"
