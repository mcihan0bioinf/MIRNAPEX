# Lasso training with high/low stratified pairing in both directions.
...
def build_X(apa_block, gene_block, apa_index, gene_index):
    """Align *_apa and *_gene feature rows to fixed indices; fill missing with 0."""
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python train_lasso_mirna.py <batch:int> <top_x_genes:int>")
        sys.exit(1)
    ...
    for idx, miRNA in enumerate(mirnas, start=1):
        ...
        # CV over alpha
        best_alpha, best_r2, best_mse = None, -np.inf, np.inf
        for alpha in alphas:
            r2s, mses = [], []
            for i in range(5):
            ...
            if avg_r2 > best_r2:
                best_r2, best_mse, best_alpha = avg_r2, avg_mse, alpha
        ...
        final = Lasso(alpha=best_alpha, max_iter=1000)
    ...
    # Save
    out_csv = out_dir / "final" / f"lasso_n_{n}_batch_{batch}_top{top_x_genes}.csv"
