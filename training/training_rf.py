# Random Forest training with high/low stratified pairing in both directions.
...
def build_X(apa_block, gene_block, apa_index, gene_index):
    """Align *_apa and *_gene rows to fixed indices; fill missing with 0."""
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python train_randomforest_mirna.py <batch:int> <top_x_genes:int>")
        sys.exit(1)
    ...
    n_estimators_values = [50, 100, 300, 500, 700]
    max_depth_values    = [None, 10, 20, 30]
    max_features_values = ["sqrt", "log2", None]  # None ≈ use all features
    ...
    for idx, miRNA in enumerate(mirnas, start=1):
        ...
        # --- 5-fold CV over (n_estimators, max_depth, max_features) ---
        best_params = {"n_estimators": None, "max_depth": None, "max_features": None}
        ...
        # Final train + test with best params
    ...
    # Save
    out_csv = out_dir / "final" / f"randomforest_n_{n}_batch_{batch}_top{top_x_genes}.csv"
